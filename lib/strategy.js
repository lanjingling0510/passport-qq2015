...
QQTokenStrategy.prototype.authenticate = function (req, options) {
  ...
  const passReqToCallback = Object.assign({}, this._passReqToCallback);
    ...
    get(url)
      ...
      .then((data) => {
        ...
        if(passReqToCallback){
            console.log("[SSO-QQ] req is set.");
            return this._verify(req,access_token, refresh_token, profile,verified);
        }else{
            console.log("[SSO-QQ] req is not set.");
            return this._verify(access_token, refresh_token, profile,verified);  
          
        }
        console.log("[SSO-QQ]passport-qq2015-fix process done!");
      }).catch(err => {
        console.warn("[SSO-QQ]Passport-qq2015-fix catch a error:");
        console.warn(err);
